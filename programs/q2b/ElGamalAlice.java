...
public class ElGamalAlice
{
	...
	private static BigInteger computeK(BigInteger p)
	{
		...
		while(k.compareTo(p_)<0)
		{
			BigInteger g = k.gcd(p_);
	...
	private static BigInteger computeB(	String message, BigInteger d, BigInteger a, BigInteger k, BigInteger p)
	{
		...
		BigInteger m_da = m.subtract(da);
		BigInteger k_ = k.modInverse(p.subtract(BigInteger.ONE));
		BigInteger b = m_da.multiply(k_);
		return b.mod(p.subtract(BigInteger.ONE));
	}
